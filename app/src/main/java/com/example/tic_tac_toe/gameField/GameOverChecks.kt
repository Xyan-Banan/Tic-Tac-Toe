...
private fun findGameOverRow(images: List<Drawable?>): Drawable? {
...
fun isGameOver(
    ...
    val gameOverBitmap = findGameOverRow(images)?.toBitmap()
    val crossBitmap = AppCompatResources.getDrawable(context, R.drawable.cross)!!.toBitmap()

    val msg =
        if (gameOverBitmap != null) {
            when (gameType) {
                GameType.SOLOCROSS -> when {
                    gameOverBitmap.sameAs(crossBitmap) -> "Game over: You win!"
                    else -> "Game over: You lose :("
                }
                GameType.SOLONOUGHT -> when {
                    gameOverBitmap.sameAs(crossBitmap) -> "Game over: You lose :("
                    else -> "Game over: You win!"
                }
                GameType.TWOPLAYERS -> when {
                    gameOverBitmap.sameAs(crossBitmap) -> "Game over: Player 1 win!"
